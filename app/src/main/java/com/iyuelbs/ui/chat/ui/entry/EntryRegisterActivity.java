...
import com.iyuelbs.ui.chat.util.NetAsyncTask;
...
public class EntryRegisterActivity extends EntryBaseActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    findView();
    initActionBar(AppApplication.getApplication().getString(R.string.register));
  ...
  private void register() {
    ...
    new NetAsyncTask(ctx) {
      ...
      protected void onPost(Exception e) {
        if (e != null) {
          Utils.toast(AppApplication.getApplication().getString(R.string.registerFailed) + e.getMessage());
        } else {
          Utils.toast(R.string.registerSucceed);
          UserService.updateUserLocation();
          MsgActivity.goMainActivityFromActivity(EntryRegisterActivity.this);
        }
      }
    }.execute();
  }
